...
class Model:
    ...
    def __init__(self, G_network, U_hom_network):
        ...
        if self.n_output != len(U_hom_network):
            raise ValueError("Output shape of the networks must match: G = (%d,%d), U_hom = (%d,)." % (
                self.n_output, self.n_input, len(U_hom_network)))
    ...
    def init_optimizer(self):
        ...
        self.train_op_Adam = self.optimizer_Adam.minimize(
            self.idn_loss.outputs, var_list=tf.trainable_variables())
    ...
    def train(self, example_path, example_name):
        ...
        tf_dict = self.idn_loss.feed_dict(
            self.x, self.y, self.f, self.u, self.weights_x, self.weights_y)
    ...
    def save_loss(self):
        ...
        np.savetxt("%s/loss_%s.csv" % (self.path_training,
                   self.activation_name), L, fmt='%.4e', delimiter=',')
    ...
    def save_results(self):
        ...
        if self.dimension == 1:
            ...
            raise ValueError(
                "Function not implemented for dimension greater than 2.")
