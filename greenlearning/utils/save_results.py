...
def save_results(model, Green_slice=1):
    ...
    for i in range(model.n_output):
        for j in range(model.n_input):
            ...
            if model.dimension == 1:
                np.savetxt('%s/Green_%s_%s_%d.csv' % (model.path_csv, model.example_name,
                           model.activation_name, k), G_pred, fmt='%.4e', delimiter=',')
            else:
                np.savetxt('%s/Green_%s_%s_%d-%d.csv' % (model.path_csv, model.example_name,
                           model.activation_name, k, Green_slice), G_pred, fmt='%.4e', delimiter=',')
        ...
        np.savetxt('%s/Hom_%s_%s_%d.csv' % (model.path_csv, model.example_name,
                   model.activation_name, i), N_pred, fmt='%.4e', delimiter=',')
