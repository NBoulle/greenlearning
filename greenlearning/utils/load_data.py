...
def load_data(model, example_path, example_name):
    ...
    # Load the dataset
    model.data_idn = scipy.io.loadmat(
        example_path + "%s.mat" % model.example_name)
    ...
    if model.G_network[0][0].layers[0] != 2 * model.dimension:
        raise ValueError("First layer of G: %d does not match dimension 2d = %d." % (
            model.G_network[0][0].layers[0], 2 * model.dimension))
    ...
    model.weights_y = np.reshape(get_weights(
        quadrature_rule, model.y), (-1, 1, 1))
    ...
    if G_shape != expected_shape:
        raise ValueError("Shape of G: (%d,%d) and training data: (%d,%d) don't match." % (
            G_shape[0], G_shape[1], expected_shape[0], expected_shape[1]))
